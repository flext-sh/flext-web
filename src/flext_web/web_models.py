...
class FlextWebApp(FlextEntity):
    ...
    def _status_enum(self) -> FlextWebAppStatus:
      ...
      """
      # Status is declared as FlextWebAppStatus, but defensively coerce
      status_field: object = self.status
      if isinstance(status_field, FlextWebAppStatus):
          return status_field
      try:
          return FlextWebAppStatus(str(status_field))
      except Exception:
          return FlextWebAppStatus.ERROR
