...
from scripts.libs.templates import GENERATED_SHELL_HEADER_TEMPLATE

GENERATED_HEADER = GENERATED_SHELL_HEADER_TEMPLATE.format(
    source="scripts/github/sync_workflows.py"
)
...
def _render_template(template_path: Path) -> str:
    body = template_path.read_text(encoding=DEFAULT_ENCODING)
...
def _sync_project(
    *,
    ...
    if destination.exists():
        current = destination.read_text(encoding=DEFAULT_ENCODING)
        if current != rendered_template:
            if apply:
                _ = destination.write_text(rendered_template, encoding=DEFAULT_ENCODING)
            operations.append(
                Operation(
