...
def test_container_pytest() -> bool:
    ...
    cmd_import = 'docker run --rm Flext-web-test python -c "import flext_web; print(\\"✅ Import successful\\"); from flext_web import FlextWebModels, FlextWebHandlers, FlextWebConfig, FlextWebServices, FlextWebTypes, FlextWebUtilities"'
    ...
    cmd_manual = 'docker run --rm Flext-web-test python -c "from flext_web import FlextWebModels, FlextWebHandlers, FlextWebConfigs, FlextWebServices; app = FlextWebModels.WebApp(id=\\"test\\", name=\\"test-app\\", port=8080, host=\\"localhost\\"); print(f\\"App created: {app.name}\\"); config = FlextWebConfig.WebConfig(secret_key=\\"test-key-32-characters-long-valid!\\"); print(f\\"Config created: {config.host}:{config.port}\\"); service = FlextWebServices.create_web_service(config); print(\\"Service created successfully\\"); print(\\"All manual tests passed in container!\\");"'
